...
（2）加载代表性数据和创建脚本：不是真的要输出文件，而是创建将在模型运行期间将要在哪输出文件。在Writers菜单下，选择提取数据的合适信息。用户应定义文件名及输出频率。文件名必须包含%t，当创建文件时，将被时间步代替。![](./media/image6.emf)
...
State控件。用户可点击跳出的初始化窗口中的Next按钮。![](./media/image8.emf)
...
![](./media/image14.emf)：表示没有Catalyst提取数据发送到服务器；如果需要发送，点击此按钮；![](./media/image15.emf)：表示已经在Paraview服务器上可获取数据了。停止提取发送给服务器的信息，可以在Paraview服务器管线中删除该对象。
