   ...
   m_refdens(Real_t(1.0))
{
   ...
   conduit_node_set_path_char8_str(m_conduit_node, "topologies/mesh/type", "unstructured");
   conduit_node_set_path_char8_str(m_conduit_node, "topologies/mesh/coordset", "coords");
   conduit_node_set_path_char8_str(m_conduit_node, "topologies/mesh/elements/shape", "hex");
   if(m_connectivity.empty())
   {
     Index_t number_points_per_edge = nx +1;
     m_connectivity.reserve((number_points_per_edge-1)*(number_points_per_edge-1)*(number_points_per_edge-1));
     Index_t x_offset = 1;
     Index_t y_offset = number_points_per_edge;
     Index_t z_offset = number_points_per_edge * number_points_per_edge;
      for(size_t k=0; k<number_points_per_edge-1;++k)
      {
        for(size_t j=0; j<number_points_per_edge-1;++j)
        {
          for(size_t i=0; i<number_points_per_edge-1;++i)
          {
            Index_t cornerIndex = i*x_offset + j*y_offset + k*z_offset;
            m_connectivity.push_back(cornerIndex);
            m_connectivity.push_back(cornerIndex + x_offset);
            m_connectivity.push_back(cornerIndex + x_offset + y_offset);
            m_connectivity.push_back(cornerIndex + y_offset);
            m_connectivity.push_back(cornerIndex + z_offset);
            m_connectivity.push_back(cornerIndex + z_offset + x_offset);
            m_connectivity.push_back(cornerIndex + z_offset + x_offset+y_offset);
            m_connectivity.push_back(cornerIndex + z_offset + y_offset);
          }
        }
      }
   }
   conduit_node_set_path_external_int32_ptr(m_conduit_node, "topologies/mesh/elements/connectivity", &m_connectivity.front(), m_connectivity.size());
   conduit_node_set_path_uint64(m_conduit_node, "topologies/mesh/number_of_cell", nx*nx*nx);
   conduit_node_set_path_int32(m_conduit_node, "topologies/mesh/cells_shape", nx*nx*nx);
   conduit_node_set_path_int32(m_conduit_node, "topologies/mesh/cells_start", 0);
   conduit_node_set_path_int32(m_conduit_node, "topologies/mesh/cells_count", nx*nx*nx);
   conduit_node_set_path_uint64(m_conduit_node, "coordsets/coords/number_of_points", (nx+1)*(nx+1)*(nx+1));
